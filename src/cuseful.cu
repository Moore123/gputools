...
char * cublasGetErrorString(cublasStatus err)
{
	switch(err) {
		case CUBLAS_STATUS_SUCCESS :
			return (char *) "operation completed successfully";
		case CUBLAS_STATUS_NOT_INITIALIZED :
			return (char *) "CUBLAS library not initialized";
		case CUBLAS_STATUS_ALLOC_FAILED :
			return (char *) "resource allocation failed";
		case CUBLAS_STATUS_INVALID_VALUE :
			return (char *) "unsupported numerical value was passed to function";
		case CUBLAS_STATUS_ARCH_MISMATCH :
			return (char *) "function requires an architectural feature absent from \
			the architecture of the device";
		case CUBLAS_STATUS_MAPPING_ERROR :
			return (char *) "access to GPU memory space failed";
		case CUBLAS_STATUS_EXECUTION_FAILED :
			return (char *) "GPU program failed to execute";
		case CUBLAS_STATUS_INTERNAL_ERROR :
			return (char *) "an internal CUBLAS operation failed";
		default :
